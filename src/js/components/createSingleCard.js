export function renderAuctionDetails(auctionData) {
  ...
  if (bidHistoryBody) {
    ...
    if (auctionData.bids && auctionData.bids.length > 0) {
      ...
      // Latest three bids displayed by default
      const latestBids = sortedBids.slice(-3).reverse(); // Most recent on top
      const olderBids = sortedBids.slice(0, -3).reverse();

      // Function to generate bid rows
      ...
      if (olderBids.length > 0) {
        ...
        // Handle viewing of remaining bids
